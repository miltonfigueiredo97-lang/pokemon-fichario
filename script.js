...
let settings={binder_name:"Meu Fichário",binder_pages:1,binder_background:"graphite",show_values:false};
...
function renderSummary(){const counts={missing:0,wanted:0,owned:0,ordered:0};collection.forEach(c=>counts[c.collection_status||"owned"]++);const total=collection.length,cap=Math.max(1,+settings.binder_pages||1)*9,empty=Math.max(0,cap-total),pct=total?Math.round(counts.owned/total*100):0,value=collection.filter(c=>(c.collection_status||"owned")==="owned").reduce((s,c)=>s+(+c.price_avg||0)*Math.max(+c.quantity||1,1),0);$("completionPercent").textContent=pct+"%";$("progressBar").style.width=pct+"%";$("progressText").textContent=`${counts.owned} de ${total} marcadas como Tenho`;for(const[k,id]of Object.entries({sumTotal:total,sumOwned:counts.owned,sumWanted:counts.wanted,sumOrdered:counts.ordered,sumMissing:counts.missing,sumEmpty:empty,filterAllCount:total,filterOwnedCount:counts.owned,filterWantedCount:counts.wanted,filterOrderedCount:counts.ordered,filterMissingCount:counts.missing,coverSlots:cap,coverOwned:counts.owned,coverWanted:counts.wanted}))$(k).textContent=id;$("totalValue").textContent=money(value);$("binderHeaderStats").textContent=`${total} cartas · ${counts.owned} tenho · ${counts.wanted} quero · ${pct}% completo`;document.querySelectorAll("[data-status-filter]").forEach(b=>b.classList.toggle("active",b.dataset.statusFilter===activeStatusFilter))}
