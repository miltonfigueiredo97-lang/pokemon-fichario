...
let settings={binder_name:"Meu Fichário",binder_pages:1,binder_background:"graphite",show_values:false,card_price_metric:"avg",total_price_metric:"avg"};
...
const PRICE_METRIC_LABEL={min:"mínimo",avg:"médio",max:"máximo"};
function normalizePriceMetric(v){return["min","avg","max"].includes(v)?v:"avg"}
function cardMetricValue(card,metric){return Number(card?.["price_"+normalizePriceMetric(metric)]||0)}
...
function applySettings(){const n=settings.binder_name||"Meu Fichário";$("binderTitleHeader").textContent=n;document.querySelector(".cover-title").innerHTML=esc(n).replace(/\s+/,"<br>");$("binderNameInput").value=n;$("binderStage").className=`binder-stage theme-${settings.binder_background||"graphite"}`;$("showValues").checked=!!settings.show_values;$("totalValueBox").classList.toggle("hidden",!settings.show_values);const cardMetric=normalizePriceMetric(settings.card_price_metric),totalMetric=normalizePriceMetric(settings.total_price_metric);if($("cardPriceMetric"))$("cardPriceMetric").value=cardMetric;if($("totalPriceMetric"))$("totalPriceMetric").value=totalMetric;if($("totalValueLabel"))$("totalValueLabel").textContent=`Valor total · preço ${PRICE_METRIC_LABEL[totalMetric]}`;document.querySelectorAll(".theme-swatch").forEach(x=>x.classList.toggle("active",x.dataset.theme===settings.binder_background));currentPage=Math.min(Math.max(1,currentPage),Math.max(1,+settings.binder_pages||1))}
...
function renderSummary(){
  ...
  collection.forEach(c=>counts[c.collection_status||"owned"]++);
  const total=collection.length,
    cap=Math.max(1,+settings.binder_pages||1)*9,
    empty=Math.max(0,cap-total),
    pct=total?Math.round(counts.owned/total*100):0,
    metric=normalizePriceMetric(settings.total_price_metric),
    value=collection
      .filter(c=>(c.collection_status||"owned")==="owned")
      .reduce((s,c)=>s+cardMetricValue(c,metric)*Math.max(+c.quantity||1,1),0);
  $("completionPercent").textContent=pct+"%";
  $("progressBar").style.width=pct+"%";
  $("progressText").textContent=`${counts.owned} de ${total} marcadas como Tenho`;
  for(const[k,id]of Object.entries({sumTotal:total,sumOwned:counts.owned,sumWanted:counts.wanted,sumOrdered:counts.ordered,sumMissing:counts.missing,sumEmpty:empty,filterAllCount:total,filterOwnedCount:counts.owned,filterWantedCount:counts.wanted,filterOrderedCount:counts.ordered,filterMissingCount:counts.missing,coverSlots:cap,coverOwned:counts.owned,coverWanted:counts.wanted}))$(k).textContent=id;
  $("totalValue").textContent=money(value);
  if($("totalValueLabel"))$("totalValueLabel").textContent=`Valor total · preço ${PRICE_METRIC_LABEL[metric]}`;
  $("binderHeaderStats").textContent=`${total} cartas · ${counts.owned} tenho · ${counts.wanted} quero · ${pct}% completo`;
  document.querySelectorAll("[data-status-filter]").forEach(b=>b.classList.toggle("active",b.dataset.statusFilter===activeStatusFilter));
}
...
function renderPocketCard(c){
  ...
  const img=cardImage(c),
    q=Math.max(0,+c.quantity||0),
    metric=normalizePriceMetric(settings.card_price_metric),
    unitValue=cardMetricValue(c,metric),
    v=unitValue*Math.max(q,1);
  b.innerHTML=`${img?`<img src="${esc(img)}" alt="${esc(c.name)}" loading="lazy" draggable="false">`:`<span>${esc(c.name)}</span>`}<span class="card-status-ribbon">${esc(STATUS[st]||st)}</span>${st==='owned'&&q>1?`<span class="card-qty">x${q}</span>`:''}${settings.show_values&&unitValue>0?`<span class="card-value" title="Preço ${PRICE_METRIC_LABEL[metric]}">${money(v)}</span>`:''}`;
