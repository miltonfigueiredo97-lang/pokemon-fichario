...
async function readProduct(page,url,options={}){
  const gotoOptions={waitUntil:'domcontentloaded',timeout:18000};
  ...
  try{
    ...
    if(url&&finishKind(wanted?.finish)!=='normal'){
      ...
      const variantPages=[
        ...
        '?estoque-cert-page=2',
        '?estoque-cert-page=3',
        '?estoque-outros-page=1',
        '?estoque-outros-page=2',
        '?estoque-outros-page=3'
      ];
      const markets=[];
      for(const suffix of variantPages){
        try{
          ...
          if(m.ok){
            markets.push(m);
            // Uma variante explícita já é suficiente; continue só até termos
            // mais de uma amostra para média/máximo mais úteis.
            if(Number(m.samples||0)>=2)return{...m,mode:'browser-seller-pagination'};
          }
        }catch{}
      }
      if(markets.length){
