...
async function readProduct(page,url,options={}){
  ...
  const gotoOptions={waitUntil:'domcontentloaded',timeout:batch?3400:(quick?6000:12000)};
...
async function resolveBatchMypLinksBrowser(items=[]){
  ...
  try{
    ...
    const contexts=await Promise.all(input.map(async item=>{
      ...
      const np=numberParts(wanted.number);
      const collector=/^\d+$/.test(np.n)?Number(np.n):0;
      const total=Math.max(Number(meta?.total||0),collector,30);
      const pageNo=collector?Math.max(1,Math.floor(Math.max(0,total-collector)/30)+1):1;
      const slugs=[...new Set([slugify(wanted.set),...(meta?.slugs||[])].filter(Boolean))];
      return{
        key,wanted,np,collector,total,pageNo,slugs,
        candidate:/^https:\/\/(?:www\.)?mypcards\.com\/pokemon\/produto\/\d+\//i.test(direct)?direct:'',
        referer:''
      };
    }));

    // Stage 1: resolve product links. Cards that share set/page share ONE browser
    // navigation instead of launching ten independent searches.
    ...
    for(const ctx of contexts){
      ...
      if(!groups.has(collectionUrl))groups.set(collectionUrl,[]);
      groups.get(collectionUrl).push(ctx);
    }

    await Promise.all([...groups.entries()].map(async([collectionUrl,members])=>{
      const page=await browser.newPage();
      try{
    ...
    // If the localized set slug was not the actual MYP slug, use the second
    // structured set slug in the same single batch attempt.
    ...
    for(const ctx of contexts){
      ...
      if(!slug)continue;
      const collectionUrl='https://mypcards.com/pokemon/'+slug+'?page='+ctx.pageNo+'&per-page=30&sort=-codigoproduto';
      if(!secondGroups.has(collectionUrl))secondGroups.set(collectionUrl,[]);
      secondGroups.get(collectionUrl).push(ctx);
    }
    await Promise.all([...secondGroups.entries()].map(async([collectionUrl,members])=>{
      const page=await browser.newPage();
      try{
        await page.goto(collectionUrl,{waitUntil:'domcontentloaded',timeout:2600});
        await page.waitForSelector('a[href*="/pokemon/produto/"]',{timeout:500}).catch(()=>{});
        const rows=await page.evaluate(()=>[...document.querySelectorAll('a[href*="/pokemon/produto/"]')].map(a=>({
          href:a.href||'',text:(a.closest('article,li,.produto-item,.produto,.card,div')?.innerText||a.innerText||'').trim()
        })));
        for(const ctx of members){
          const exact=rows.filter(row=>{
            const found=numberParts(row.text||'');
          ...
          exact.sort((a,b)=>searchCandidateScore(b,ctx.wanted)-searchCandidateScore(a,ctx.wanted));
          if(exact.length){ctx.candidate=exact[0].href;ctx.referer=collectionUrl}
        }
      }catch{}finally{await page.close().catch(()=>{})}
    }));

    // Stage 2: all product pages are read concurrently in the SAME batch.
    const results=await Promise.all(contexts.map(async ctx=>{
      if(!ctx.candidate){
        return{key:ctx.key,name:ctx.wanted.name,number:ctx.wanted.number,ok:false,error:'batch_exact_product_not_found',elapsedMs:Date.now()-started};
      }
      const page=await browser.newPage();
      try{
