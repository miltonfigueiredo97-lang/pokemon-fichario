...
function summarizeProduct(data,wanted){
  ...
  // representa a mesma impressão Foil, mas nunca misturamos Full-Art/Reverse/etc.
  ...
  return{
    ok:true,provider:'Chromium',mode:'browser-page',title:data.title,edition:data.edition,link:data.url,httpStatus:data.httpStatus,
    min,avg,max,samples:prices.length,availableQuantity:qty||null,exactVariant:true,complete:!!(min&&avg&&max),
...
async function discoverProduct(browser,page,wanted){
  ...
  for(const candidate of candidates){
    ...
    try{
      // Cada impressão é aberta isoladamente. Uma candidata bloqueada por
      // Cloudflare não pode contaminar a navegação das candidatas seguintes.
      candidatePage=await configurePage(await browser.newPage());
      const data=await readProduct(candidatePage,candidate.href);
      ...
      if(candidatePage)await candidatePage.close().catch(()=>{});
    }
  }
  return best;
}
