...
async function readProduct(page,url){
  ...
  await page.waitForSelector('td.estoque-lista-precoestoque',{timeout:6500}).catch(()=>{});
...
async function resolveBrowserSetMeta(wanted){
  ...
  if(wanted?.apiId){
    try{
      const r=await fetch('https://api.tcgdex.net/v2/en/cards/'+encodeURIComponent(wanted.apiId),{
        headers:{accept:'application/json','user-agent':'PokemonBinderBR-BrowserIndex/1'}
      });
      if(r.ok){
  ...
  for(const slug of meta.slugs.slice(0,3)){
    for(const pageNo of [...new Set(pages)].slice(0,1)){
      try{
        ...
        await page.goto(collectionUrl,{waitUntil:'domcontentloaded',timeout:12000});
        ...
        const raw=await page.evaluate(()=>[...document.querySelectorAll('a[href*="/pokemon/produto/"]')].map(a=>({
          href:a.href,
          text:(a.closest('article,li,.produto-item,.produto,.card,.item,.row,div')?.innerText||a.innerText||'').trim()
        })));
...
async function findAndScrapeMypBrowser(url,wanted={}){
  ...
  try{
    ...
    if(url)data=await readProduct(page,url);
      ...
      if(!data)return{ok:false,error:'collection_not_found',stage:'collection',elapsedMs:Date.now()-started,message:'A página da coleção não localizou a carta pelo número.'};
    }
