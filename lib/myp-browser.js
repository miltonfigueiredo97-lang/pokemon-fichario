...
async function resolveBatchMypLinksBrowser(items=[]){
  const wantedItems=(Array.isArray(items)?items:[]).slice(0,10);
  if(!wantedItems.length)return{ok:true,items:[]};
  let browser;
  const started=Date.now();
  try{
    const launched=await launch();
    ...
    const getMeta=async(wanted)=>{
      const key=String(wanted.setId||wanted.set||'').toLowerCase();
      if(!metaCache.has(key))metaCache.set(key,resolveBrowserSetMeta(wanted).catch(()=>({total:0,slugs:[]})));
      return await metaCache.get(key);
    };

    const results=await Promise.all(wantedItems.map(async(item)=>{
      const page=await browser.newPage();
      const wanted={
      ...
      try{
        ...
        let candidate=/^https:\/\/(?:www\.)?mypcards\.com\/pokemon\/produto\/\d+\//i.test(direct)?direct:'';
        let referer='';

        if(!candidate){
          const meta=await getMeta(wanted);
          const np=numberParts(wanted.number);
          const collector=/^\d+$/.test(np.n)?Number(np.n):0;
          ...
          const pages=[...new Set([estimated,estimated>1?estimated-1:null,estimated+1].filter(Boolean))].slice(0,3);
          ...
          for(const slug of slugs){
            for(const pageNo of pages){
              ...
              try{
                await page.goto(collectionUrl,{waitUntil:'domcontentloaded',timeout:3600});
                await page.waitForSelector('a[href*="/pokemon/produto/"]',{timeout:700}).catch(()=>{});
                const rows=await page.evaluate(()=>[...document.querySelectorAll('a[href*="/pokemon/produto/"]')].map(a=>{
                  let node=a,best=(a.innerText||'').trim();
                  for(let d=0;d<9&&node;d++,node=node.parentElement){
                    const text=(node.innerText||'').trim();
        ...
        // Same single attempt, fallback only when the deterministic set page did
        // not expose the exact collector number. It is not a retry/requeue.
        if(!candidate){
          try{
            const query=['site:mypcards.com/pokemon/produto',wanted.name,wanted.number,wanted.set].filter(Boolean).join(' ');
            await page.goto('https://www.google.com/search?q='+encodeURIComponent(query),{waitUntil:'domcontentloaded',timeout:3000});
            const links=await page.evaluate(()=>[...document.querySelectorAll('a[href]')].map(a=>a.href||'').map(h=>{
              try{
                const u=new URL(h);
                if(u.hostname.includes('google.')&&u.pathname==='/url')return u.searchParams.get('q')||u.searchParams.get('url')||'';
                return h;
            ...
            candidate=[...new Set(links)][0]||'';
          }catch{}
        }

        if(!candidate){
          return{key,name:wanted.name,number:wanted.number,ok:false,error:'batch_exact_product_not_found',link:'',elapsedMs:Date.now()-started};
        }

        const data=await readProduct(page,candidate,{quick:true,referer:referer||undefined});
        if(Number(data.httpStatus)>=400||!productIdentityOk(data,wanted)){
          return{key,name:wanted.name,number:wanted.number,ok:false,error:'batch_wrong_product',link:candidate,elapsedMs:Date.now()-started};
        }
        ...
        return{
          ...
          ...summary,
          link:String(data.url||candidate),
          edition:data.edition||wanted.set||'',
          mode:'browser-batch10-set-page',
          elapsedMs:Date.now()-started
        };
      }catch(error){
        return{key,name:wanted.name,number:wanted.number,ok:false,error:'batch_item_error',message:String(error?.message||error||''),elapsedMs:Date.now()-started};
      }finally{
        await page.close().catch(()=>{});
      }
    }));
