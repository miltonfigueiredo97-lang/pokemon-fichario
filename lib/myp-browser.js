...
async function discoverProduct(browser,page,wanted){
  ...
  const queries=[...new Set([
    ...names.flatMap(n=>[
      ...setAliases.map(s=>[n,collector,s].filter(Boolean).join(' ')),
      ...setAliases.map(s=>[n,shortCollector,s].filter(Boolean).join(' ')),
      [n,collector].filter(Boolean).join(' '),
      [n,shortCollector].filter(Boolean).join(' '),
      n
    ]),
    ...setAliases.map(s=>[collector,s].filter(Boolean).join(' ')),
    collector
  ...
  for(const query of queries){
    ...
    try{
      ...
      await page.waitForSelector('a[href*="/pokemon/produto/"]',{timeout:5000}).catch(()=>{});
...
async function findAndScrapeMypBrowser(url,wanted={}){
  ...
  try{
    ...
    let data=url?await readProduct(page,url):await discoverProduct(browser,page,wanted);
