  ...
  try{
    ...
    if(url&&finishKind(wanted?.finish)!=='normal'){
      ...
      for(let sellerPage=2;sellerPage<=5;sellerPage++){
        variantPages.push('?estoque-outros-page='+sellerPage);
        variantPages.push('?estoque-cert-page='+sellerPage);
      }
      const markets=[];
      ...
      for(const suffix of variantPages){
        try{
          ...
          pageDiagnostics.push({suffix,error:String(error?.message||error||'page_error')});
        }
      }
