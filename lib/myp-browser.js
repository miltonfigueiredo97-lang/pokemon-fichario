...
function summarizeProduct(data,wanted){
  ...
  // A página já representa uma impressão específica da carta. Na MYP muitos
  // vendedores deixam o campo acabamento vazio quando vendem o acabamento
  // padrão daquela impressão. Portanto:
  // - a condição SEMPRE precisa bater;
  // - acabamento vazio conta como a impressão padrão da página;
  // - acabamento explicitamente diferente (Full-Art, Reverse etc.) é excluído.
  ...
  return{
    ...
    finish:wantedFinish,
    matched:matched.map(x=>({seller:x.seller,finish:x.finish||'(padrão da impressão)',condition:x.condition,price:priceFrom(x.price),qty:x.qty}))
  };
}
