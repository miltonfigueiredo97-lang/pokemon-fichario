...
async function launch(){
  ...
  // V16.46: use the full stealth evasions rather than hand-written navigator
  // patches only. MYP's Cloudflare was identifying the Vercel Chromium before
  // the exact search/product page could load.
  let puppeteer=puppeteerCore;
  try{
    const extraModule=await import('puppeteer-extra');
    const stealthModule=await import('puppeteer-extra-plugin-stealth');
    const addExtra=extraModule.addExtra||extraModule.default?.addExtra;
    ...
    console.warn('[MYP browser stealth]',error?.message||error);
  }
