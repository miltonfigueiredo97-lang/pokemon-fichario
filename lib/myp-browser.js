...
function conditionKind(v){
  ...
  if(!n||n==='NOVA'||n==='NEW'||n==='MINT'||/\bNEAR MINT\b/.test(n)||/^NM\b/.test(n))return'NM';
  if(/\bSLIGHTLY PLAYED\b/.test(n)||/^SP\b/.test(n))return'SP';
  if(/\bMODERATELY PLAYED\b/.test(n)||/^MP\b/.test(n))return'MP';
  if(/\bHEAVILY PLAYED\b/.test(n)||/^HP\b/.test(n))return'HP';
  if(/\bDAMAGED\b/.test(n)||/^DM\b/.test(n)||n==='D')return'DM';
  return n;
}
