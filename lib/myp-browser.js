...
async function resolveBatchMypLinksBrowser(items=[]){
  ...
  try{
    ...
    const results=await Promise.all(wantedItems.map(async(item)=>{
      ...
      try{
        const query=['site:mypcards.com/pokemon/produto',wanted.name,wanted.number].filter(Boolean).join(' ');
        const searchUrl='https://www.google.com/search?q='+encodeURIComponent(query);
        let candidates=[];
        try{
          await page.goto(searchUrl,{waitUntil:'domcontentloaded',timeout:5500});
          candidates=await page.evaluate(()=>[...document.querySelectorAll('a[href]')]
            .map(a=>a.href||'')
            .map(h=>{
              try{
        ...
        // If Google did not return a usable result, use MYP's own exact search
        // in the SAME browser/page; this is still the same single batch attempt.
        if(!candidates.length){
          const internal='https://mypcards.com/pokemon?ProdutoSearch%5Bmarca%5D=pokemon&ProdutoSearch%5Bquery%5D='+
            encodeURIComponent([wanted.name,wanted.number].filter(Boolean).join(' '));
          try{
            await page.goto(internal,{waitUntil:'domcontentloaded',timeout:4500});
            await page.waitForSelector('a[href*="/pokemon/produto/"]',{timeout:1400}).catch(()=>{});
            const rows=await page.evaluate(()=>[...document.querySelectorAll('a[href*="/pokemon/produto/"]')].map(a=>({
              href:a.href||'',
              text:(a.closest('article,li,.produto-item,.produto,.card,div')?.innerText||a.innerText||'').trim()
            })));
            ...
            const filtered=rows.filter(row=>{
              ...
              const np=numberParts(row.text);
              return !wantedNum.n||(np.n===wantedNum.n&&(!wantedNum.d||!np.d||np.d===wantedNum.d));
            });
            filtered.sort((a,b)=>searchCandidateScore(b,wanted)-searchCandidateScore(a,wanted));
            candidates=[...new Set(filtered.map(x=>x.href))].slice(0,3);
          }catch{}
        }
    ...
    return{ok:false,error:'batch_link_browser_error',message:error?.message||String(error),items:[]};
  }finally{
    if(browser)await browser.close().catch(()=>{});
  }
}
