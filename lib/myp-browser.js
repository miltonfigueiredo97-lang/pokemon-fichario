...
function productLocaleScore(data,wanted){
  ...
  const hay=normalize([data?.title,data?.edition,data?.body].filter(Boolean).join(' '));
...
function productIdentityOk(data,wanted){
  if(!identityOk(data?.title,wanted))return false;
...
function summarizeProduct(data,wanted){
  ...
  const matched=sameCondition.filter(row=>{
    ...
    // Linha sem acabamento é a impressão padrão/Normal. Ela nunca pode
    // contaminar Reverse, Foil, Poké Ball, Master Ball ou Altered Art.
    if(!raw)return wantedFinish==='normal';
