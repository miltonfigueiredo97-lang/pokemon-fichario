  ...
  for(const slug of meta.slugs.slice(0,3)){
    for(const pageNo of [...new Set(pages)].slice(0,1)){
      ...
      try{
        ...
        diagnostics.push({slug,pageNo,status:response?.status()||0,anchors:pageInfo.anchors,hasNumber:pageInfo.hasNumber,exact:exactCount,url:pageInfo.url});
  ...
  try{
    ...
    if(url)data=await readProduct(page,url);
      ...
      if(data?.__collectionMiss){
        const diag=(data.diagnostics||[]).map(x=>[x.slug,x.pageNo,x.status||x.error,x.anchors??'',x.hasNumber?'num':'nonum',x.exact??'',x.url||''].join(',')).join(';');
