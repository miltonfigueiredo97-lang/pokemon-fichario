...
function conditionKind(v){
  ...
  if(!n||n==='NOVA'||n==='NEW'||n==='MINT'||/\bNEAR MINT\b/.test(n)||/^NM(?:\+|-)?$/.test(n))return'NM';
  if(/\bSLIGHTLY PLAYED\b/.test(n)||/^SP(?:\+|-)?$/.test(n))return'SP';
  if(/\bMODERATELY PLAYED\b/.test(n)||/^MP(?:\+|-)?$/.test(n))return'MP';
  if(/\bHEAVILY PLAYED\b/.test(n)||/^HP(?:\+|-)?$/.test(n))return'HP';
  if(/\bDAMAGED\b/.test(n)||/^DM(?:\+|-)?$/.test(n)||n==='D')return'DM';
  return n;
}
function languageKind(v){
  const n=normalize(v);
  if(!n)return'';
  if(n==='pt'||n==='pt br'||n.includes('portugues')||n.includes('portuguese')||n==='br')return'pt-br';
  if(n==='en'||n.includes('ingles')||n.includes('english')||n==='us'||n==='gb')return'en';
...
function identityOk(title,wanted){
  const t=normalize(title),name=normalize(cleanName(wanted?.name));
  if(name&&!t.includes(name)&&!name.includes(t.replace(/[a-z]*\d+[a-z]*\s*[a-z]*\d+[a-z]*$/i,'')))return false;
  return numberIdentityMatches(title,wanted?.number);
}
...
async function readProduct(page,url){
  ...
  const data=await page.evaluate(()=>({
    ...
    offers:[...document.querySelectorAll('tr')].map(tr=>({
      ...
      language:tr.querySelector('.estoque-lista-qualidadenome .flag-icon')?.getAttribute('title')?.trim()||'',
...
function summarizeProduct(data,wanted){
  ...
  // A página da MYP mistura ofertas de vários idiomas para a mesma impressão.
  // Primeiro filtramos pelo idioma da carta salva no fichário; depois condição
  // e acabamento. Assim uma carta PT-BR nunca usa preço de Chinês/Inglês/Japonês.
  const sameLanguage=wantedLanguage
    ? data.offers.filter(row=>languageKind(row.language)===wantedLanguage)
    : data.offers;
  const sameCondition=sameLanguage.filter(row=>conditionKind(row.condition)===wantedCondition);
  const matched=sameCondition.filter(row=>{
    const raw=String(row.finish||'').trim();
    if(!raw)return true;
    const rowFinish=finishKind(raw);
  ...
  if(!prices.length){
    return{
      ...
      message:'Nenhuma oferta da MYP corresponde ao idioma + condição + acabamento selecionados.'
    };
  }
