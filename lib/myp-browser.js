...
async function searchExactMypBrowser(wanted={}){
  ...
  try{
    ...
    return{
      ok:false,error:'exact_product_not_found',
      mode:'browser-human-myp-search',
      searchQuery:query,
      message:'url='+String(diag.url||'')+' | anchors='+String(diag.anchors||0)+' | '+String(diag.title||'')+' | '+String(diag.body||'').replace(/\s+/g,' ').slice(0,180)
    };
  }catch(error){
