...
async function deriveJapaneseTerms(name){
  ...
  for(const lang of ['en','pt']){
    try{
      const p=new URLSearchParams({name:raw,'pagination:itemsPerPage':'14'});
  ...
  const candidates=[...ids.values()].sort((a,b)=>b.score-a.score).slice(0,12);
  const translated=await Promise.all(candidates.map(async row=>{
    try{
      ...
      return j?.name?{ja:String(j.name),display:row.display,score:row.score}:null;
    }catch{return null}
  }));

  const out=[],seen=new Set();
  for(const row of translated.filter(Boolean)){
    const k=norm(row.ja);
...
function parseDetail(html,id,displayName=''){
  ...
  const image=absolute(attr(imageTag,'src'));
  ...
  const collector=sub.match(/([\p{L}\p{N}-]+)\s*\/\s*([\p{L}\p{N}-]+)/u);
...
async function searchOfficialJapaneseCards({name='',number='',set='',limit=40,hp='',rarity=''}={}){
  ...
  const terms=await deriveJapaneseTerms(name);
