...
async function queryMyp(wanted){
  ...
  const alias=[...aliases].reverse().find(x=>norm(x)!==norm(base))||base;
  // A busca da MYP é por nome. Número/coleção/idioma/acabamento são
  // validados localmente depois; adicioná-los à query faz cartas válidas
  // desaparecerem do resultado (ex.: Spearow 021/165 MEW).
  const query=alias||base||String(wanted.number||'').trim();
  const input={
    query,
    game:'pokemon',
    language:langMap(wanted.lang),
