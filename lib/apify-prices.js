...
async function queryMyp(wanted){
  ...
  const setId=norm(wanted.setId||'');
  const setCode=(setId==='sv03 5'||setId==='sv3 5')?'MEW':'';
  const query=[alias,String(wanted.number||'').trim(),setCode].filter(Boolean).join(' ');
  const input={
    query,
    game:'pokemon',
    language:langMap(wanted.lang),
  ...
  const items=await runActor('gio21~mypcards-scraper',input,24);
