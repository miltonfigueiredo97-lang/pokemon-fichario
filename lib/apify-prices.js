...
async function queryMyp(wanted){
  ...
  // Para cartas localizadas, a página da MYP frequentemente mantém o título
  // inglês enquanto as ofertas dentro dela possuem bandeira PT-BR.
  // Prefira um alias diferente do nome salvo (normalmente o alias EN da TCGdex).
  ...
  const defaultAlias=[...differentAliases].reverse()[0]||base;

  // Não acrescente número a todas as buscas: isso já causou regressão em
  // cartas comuns como Spearow. Use número exato apenas em famílias realmente
  // ambíguas (ex.: Energias), onde o nome sozinho representa muitas impressões.
  const aliasHay=aliases.map(norm).join(' ');
  const ambiguousFamily=/\b(?:energia|energy|energie)\b/.test(aliasHay);
  const localizedAlias=language==='portuguese'?(differentAliases[0]||base):defaultAlias;
  const query=ambiguousFamily&&number
    ? [localizedAlias||base,number].filter(Boolean).join(' ')
    : (defaultAlias||base||number);
  ...
  const items=await runActor('gio21~mypcards-scraper',input,30);
  const rows=sourceRows(items,wanted);
  const market=summarize(rows,wanted);
  const link=rows.map(itemUrl).find(Boolean)||'';
  return Object.assign({source:'MYP Cards',provider:'Apify',link,rows:rows.length,queryUsed:query},market);
}
