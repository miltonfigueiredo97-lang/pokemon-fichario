...
async function queryMyp(wanted){
  ...
  // Usually the first exact query resolves it. Cap at four Actor calls so a
  // pathological card cannot monopolize the worker forever.
  for(const query of queryVariants.slice(0,4)){
    let items=[];
    try{items=await runActor('gio21~mypcards-scraper',makeInput(query),24)}
    catch(error){
  ...
  return Object.assign({
    source:'MYP Cards',provider:'Apify',link:bestLink,rows:rows.length,
    queryUsed:queryVariants[0]||base,queriesTried:queryVariants.slice(0,4),
