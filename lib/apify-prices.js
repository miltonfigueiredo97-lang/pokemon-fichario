...
function filterOffers(rows,wanted){
  ...
  if(explicitFinish.length){
    // Rows with an empty version are the default version of the already-selected
    // MYP product page. Explicitly different versions (Altered Art, Reverse, etc.)
    // must never contaminate the requested finish.
    ...
    exactVariant=allowed.some(x=>String(itemFinish(x)||'').trim()&&finishMatch(itemFinish(x),wantedFinish)!==false);
    filtered=allowed;
  }
...
async function queryMyp(wanted){
  ...
  const query=[...aliases].reverse().find(x=>norm(x)!==norm(base))||base||String(wanted.number||'').trim();
  const input={
    query,
    game:'pokemon',
    // Não filtre o PRODUTO pelo idioma. O idioma correto é filtrado nas
    // ofertas retornadas, pois uma página inglesa pode conter cópias PT-BR.
    language:'any',
