...
function sourceRows(items,wanted){
  ...
  // Nomes retornados pelos marketplaces quase sempre carregam sigla da edição
  // e número ("M charizard EX FLF (69/106)"). Um match parcial de nome vale 260;
  // o piso antigo de 300 descartava exatamente esses resultados válidos.
  return scored.filter(x=>x.score>=Math.max(180,top-140)).map(x=>x.item);
}
...
async function queryLiga(wanted){
  const input={
    // O Actor documenta "query" como busca textual por nome. Enviar o
    // collector number junto reduzia o resultado a zero em várias cartas.
    // Buscamos pelo nome e fazemos o desempate exato por número/edição abaixo.
    query:String(wanted.name||'').trim(),
    language:langMap(wanted.lang),
    includeOffers:true,
    condition:ligaCondition(wanted.condition),
    maxProducts:6,
    maxItems:80,
    // A própria documentação do Actor exige proxy residencial brasileiro:
    // LigaPokemon bloqueia IPs estrangeiros/datacenter via Cloudflare.
...
async function queryMyp(wanted){
  const input={
    // MYP Actor: pesquisar pelo nome; número/edição são validados localmente.
    query:String(wanted.name||'').trim(),
    game:'pokemon',
    language:langMap(wanted.lang),
