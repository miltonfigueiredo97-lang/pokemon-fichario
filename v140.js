...
(()=>{
  ...
  function wishlistBinder(){
    return V14.binders.find(b=>b.binder_kind==='wishlist')||null;
  }
  function isWishlistBinder(b=activeBinder()){
    return !!b&&b.binder_kind==='wishlist';
  }
  async function ensureWishlistBinder(){
    ...
    // Migra automaticamente nomes antigos usados antes da Lista de Desejos oficial.
    ...
    if(legacy){
      const {data,error}=await db.from('pokemon_binders')
        ...
        .eq('id',legacy.id).eq('user_id',currentUser.id)
        .select('*').single();
  ...
  function injectUI(){
    ...
    if(binderControls&&!byId('v14RenameBinder')){
      ...
      renameButton.id='v14RenameBinder';
      renameButton.className='v14-rename-binder';
      renameButton.type='button';
      renameButton.setAttribute('aria-label','Renomear fichário');
      renameButton.title='Renomear fichário';
      renameButton.textContent='✎ Renomear fichário';
  ...
  function renderBinderControls(){
    ...
    if(sel){
      sel.innerHTML='<option value="all">Geral — todos os fichários</option><option value="favorites">★ Favoritas</option>'+
        V14.binders.map(b=>'<option value="'+esc(b.id)+'">'+(b.binder_kind==='wishlist'?'$ ':'')+esc(b.name)+'</option>').join('');
    ...
    if(rename){
      rename.disabled=isGeneral()||isWishlistBinder();
      rename.classList.toggle('hidden',isGeneral()||isWishlistBinder());
    }
    const del=byId('v14DeleteBinder');
    if(del){
      del.disabled=isGeneral()||isWishlistBinder();
      del.classList.toggle('hidden',isGeneral()||isWishlistBinder());
    }
  ...
  function nextWishlistPosition(wishlist){
    const used=new Set(
      V14.allCards
        .filter(x=>x.binder_id===wishlist.id)
        .map(x=>String(+x.binder_page||1)+':'+String(+x.binder_slot||1))
    );
    const maxPages=Math.max(1,+wishlist.pages||1);
    for(let page=1;page<=maxPages;page++){
      for(let slot=1;slot<=9;slot++){
        if(!used.has(page+':'+slot))return{page,slot};
      }
    }
    return{page:maxPages+1,slot:1};
  }

  async function toggleWishlistCard(card){
    if(!card?.id||isWishlistBinder())return;
    let wishlist=wishlistBinder()||await ensureWishlistBinder();
    ...
    if(existing){
      ...
      V14.allCards=V14.allCards.filter(x=>x.id!==existing.id);
      collection=physicalCollection();
      renderAll();
    ...
    if(pos.page>(+wishlist.pages||1)){
      const {error:pageError}=await db.from('pokemon_binders')
        .update({pages:pos.page,updated_at:new Date().toISOString()})
        .eq('id',wishlist.id).eq('user_id',currentUser.id);
    ...
    const payload={
      ...
      binder_page:pos.page,
      binder_slot:pos.slot,
      price_min:+card.price_min||0,
      price_avg:+card.price_avg||0,
      price_max:+card.price_max||0,
      currency:card.currency||'BRL',
      price_source:card.price_source||'',
      price_link:card.price_link||null,
      price_br_source:card.price_br_source||null,
      price_br_link:card.price_br_link||null,
      liga_price_min:+card.liga_price_min||0,
      liga_price_avg:+card.liga_price_avg||0,
      liga_price_max:+card.liga_price_max||0,
      liga_price_link:card.liga_price_link||null,
      liga_price_checked_at:card.liga_price_checked_at||null,
      myp_price_min:+card.myp_price_min||0,
      myp_price_avg:+card.myp_price_avg||0,
      myp_price_max:+card.myp_price_max||0,
      myp_price_link:card.myp_price_link||null,
      myp_price_checked_at:card.myp_price_checked_at||null,
      market_internal_code:card.market_internal_code||null,
      market_name_pt:card.market_name_pt||null,
      market_edition_pt:card.market_edition_pt||null,
      market_image_pt:card.market_image_pt||null,
      market_image_en:card.market_image_en||null,
      finish_confirmed:!!card.finish_confirmed,
      binder_id:wishlist.id,
      price_pending:!!card.price_pending,
      price_processing_at:null,
      price_requested_at:card.price_requested_at||new Date().toISOString(),
      price_next_retry_at:card.price_next_retry_at||new Date().toISOString(),
      price_attempts:+card.price_attempts||0,
      price_priority:+card.price_priority||0,
      price_last_error:card.price_last_error||null,
      notes:card.notes||''
    };
    ...
    collection=physicalCollection();
    renderAll();
    toast('Adicionada à Lista de Desejos.');
  }
  ...
  async function addSelectedFast(){
    ...
    try{
      ...
      for(let i=0;i<cards.length;i++){
        ...
        payload.collection_status=wishlistMode?'wanted':'owned';
        payload.quantity=wishlistMode?0:1;
        payload.price_pending=true;
        payload.price_checked_at=null;
        payload.price_processing_at=null;
  ...
  function enhanceSelectionTrayV1493(){
    const tray=document.querySelector('#addDialog .selection-tray');
    const box=byId('v122FinishBox');
    if(!tray||!box)return;
    if(!box.dataset.v1493){
      box.dataset.v1493='1';
      box.classList.add('v1493-collapsed');
      box.addEventListener('click',e=>{
        if(e.target.closest('select,button,input'))return;
        if(!e.target.closest('.v122-finish-title'))return;
        box.classList.toggle('v1493-collapsed');
      });
    }
    ...
    if(title&&!title.querySelector('.v1493-selection-toggle')){
      ...
      toggle.type='button';
      toggle.className='v1493-selection-toggle';
      toggle.setAttribute('aria-label','Abrir ou minimizar cartas selecionadas');
      toggle.innerHTML='<span>Ver seleção</span><b>⌃</b>';
      toggle.onclick=e=>{
        e.preventDefault();e.stopPropagation();
        box.classList.toggle('v1493-collapsed');
        toggle.querySelector('span').textContent=box.classList.contains('v1493-collapsed')?'Ver seleção':'Minimizar';
      };
  ...
  function wireSelectionTrayV1493(){
    const tray=document.querySelector('#addDialog .selection-tray');
    if(!tray||tray.dataset.v1493)return;
    tray.dataset.v1493='1';
    new MutationObserver(()=>setTimeout(enhanceSelectionTrayV1493,0)).observe(tray,{childList:true,subtree:true});
...
;
      ...
      wish.addEventListener('click',wishAct);
      wish.addEventListener('pointerdown',e=>e.stopPropagation());
