-- V16.33: honor the worker's requested queue width.
-- p_limit=1 is used by the current reliability mode so only one card is
-- claimed at a time. The homogeneous set/language grouping is preserved.
...
begin
  ...
  order by (count(*) >= take_count) desc,
           count(*) desc,
