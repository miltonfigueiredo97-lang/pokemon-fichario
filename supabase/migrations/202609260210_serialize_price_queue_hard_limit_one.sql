-- V16.35: reliability mode. The price queue is physically serialized in
-- PostgreSQL so no worker/frontend version can accidentally claim 10 cards.
create or replace function public.claim_pokemon_price_batch(p_limit integer default 1)
...
begin
  ...
  order by max(p.price_priority) desc nulls last,
